...
if (
    isset($_POST['make_withdrawal']) and
    $_POST['make_withdrawal'] == 'make_withdrawal'
) {
    $sql = "SELECT * FROM account1_tbl WHERE username = '$username'";
    $stmt = $pdo->query($sql);
    $row = $stmt->fetch();

    $new_balance = $row['account_balance'] - $_POST['amount'];

    if ($new_balance < 0) {
        $msg = 'Insufficient balance';
    } else {
        $sql =
            'UPDATE account1_tbl SET account_balance = account_balance- ' .
            $_POST['amount'] .
...
<section id="withdraw" class="section hidden">
...
<div class="card">
    ...
    <form action="make_withdrawal.php" method="post">
    <label for="amount">How much to withdraw</label>
    <input type="text" id="amount" name="amount" required>
    <br>
    <input type="submit" name="make_withdrawal" value="make_withdrawal">
</form>
</div>
