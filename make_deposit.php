...
if (
    isset($_POST['make_deposit']) and
    $_POST['make_deposit'] == 'make_deposit'
) {
    $sql = "SELECT * FROM account1_tbl WHERE username = '$username'";
    $stmt = $pdo->query($sql);
    $row = $stmt->fetch();

    $new_balance = $_POST['amount'] + $row['account_balance'];
...
<li><a href="log out.php">log out</a></li>
   ...
   <h3>     
    ...
    <div class="card">
    <form action="make_deposit.php" method="post">
    <label for="amount">How much to deposit</label>
    <input type="number" id="amount" name="amount" required placeholder="enter the amount you want to deposit">
    <br>
    <input type="submit" name="make_deposit" value="make_deposit">
</form>
</div>
